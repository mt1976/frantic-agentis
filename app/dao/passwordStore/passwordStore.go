...
func Count() (int, error) {
	logHandler.DatabaseLogger.Printf("[%v] Count %v", appName, domain)
	return activeDB.Count(&Password_Store{})
}

func CountWhere(field string, value any) (int, error) {
	logHandler.DatabaseLogger.Printf("[%v] Count %v where (%v=%v)", appName, domain, field, value)
...
func GetBy(field string, value any) (Password_Store, error) {
	...
	if err := dao.IsValidFieldInStruct(field, Password_Store{}); err != nil {
		return Password_Store{}, err
	}

	record := Password_Store{}
	logHandler.DatabaseLogger.Printf("[%v] Get %v where (%v=%v)", appName, domain, field, value)

	if err := activeDB.Retrieve(field, value, &record); err != nil {
		clock.Stop(0)
		return Password_Store{}, commonErrors.WrapRecordNotFoundError(domain, field, fmt.Sprintf("%v", value))
	}

	if err := record.PostGet(); err != nil {
		clock.Stop(0)
		return Password_Store{}, commonErrors.WrapDAOReadError(domain, field, value, err)
	}
...
func Export(message string) {
	...
	recordList, _ := GetAll()
	if len(recordList) == 0 {
		logHandler.WarningLogger.Printf("[%v] %v data not found", appName, domain)
		clock.Stop(0)
		return
	}
	SEP := "!"
...
func Drop() error {
	return activeDB.Drop(Password_Store{})
}
...
func ExportCSV() error {
	...
	if err != nil {
		logHandler.ExportLogger.Panicf("[%v] Error Getting all %v's: %v", appName, domain, err.Error())
	}
...
func ImportCSV() error {
	return importExportHelper.ImportCSV(domain, &Password_Store{}, importProcessor)
}
