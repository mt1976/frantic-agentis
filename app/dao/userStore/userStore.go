...
func PostGet(recordList *[]User_Store) ([]User_Store, error) {
...
func (s *User_Store) PostGet() error {
...
func (record *User_Store) Export(name string) {
...
func Worker(j jobs.Job, db *database.DB) {
	...
	if db != nil {
		if activeDB.Name != db.Name {
			logHandler.EventLogger.Printf("[%v] Switching to %v.db", appName, db.Name)
			activeDB = db
			dbSwitched = true
		}
	}
	...
	if dbSwitched {
		logHandler.EventLogger.Printf("[%v] Switching back to %v.db from %v.db", appName, oldDB.Name, activeDB.Name)
		activeDB = oldDB
	}
	clock.Stop(1)
}
